...
function renderViewer() {
  ...
  el.viewerPos.textContent = `${state.viewerIndex + 1} of ${
    state.viewerIds.length
  }`;
  el.favBtn.textContent = photo.favourite
    ? "Remove from favourites"
    : "Add to favourites";
  el.favBtn.classList.toggle("hidden", state.album === "Private");
  $("coverBtn").classList.toggle("hidden", state.album === "Private");
  applyTransform();
}
...
function setZoom(value) {
  state.zoom = Math.min(5, Math.max(1, value));
...
function applyTransform() {
  el.viewerImg.style.transform = `translate(${state.panX}px, ${
    state.panY
  }px) scale(${state.zoom})`;

  $("resetZoomBtn").textContent = `${state.zoom.toFixed(
    state.zoom % 1 ? 1 : 0
  )}×`;
}
...
function touchMove(event) {
  ...
  if (event.touches.length === 2 && state.pinchDistance) {
    setZoom(
      (state.pinchZoom *
        distance(event.touches[0], event.touches[1])) /
        state.pinchDistance
    );
  } else if (
    ...
    state.panX = event.touches[0].clientX - state.drag.x;
    state.panY = event.touches[0].clientY - state.drag.y;
    applyTransform();
  }
}
...
async function setCover() {
  ...
  if (!photo || !state.album) return;
  if (state.album === "Private") {
    toast("Private Vault cover is locked");
    return;
  }
...
function bind() {
  ...
  $("viewerMenuBtn").onclick = () =>
    el.viewerMenu.classList.toggle("hidden");

  el.viewer.onclick = (event) => {
    if (
      !el.viewerMenu.contains(event.target) &&
      event.target.id !== "viewerMenuBtn"
    ) {
      el.viewerMenu.classList.add("hidden");
    }
  };

  $("prevBtn").onclick = () => moveViewer(-1);
  $("nextBtn").onclick = () => moveViewer(1);
  $("zoomOutBtn").onclick = () => setZoom(state.zoom - 0.5);
  $("zoomInBtn").onclick = () => setZoom(state.zoom + 0.5);
  ...
  el.viewerStage.addEventListener("touchstart", touchStart, { passive: true });
  el.viewerStage.addEventListener("touchmove", touchMove, { passive: false });
  el.viewerStage.addEventListener("touchend", touchEnd, { passive: true });
  ...
  $("coverBtn").onclick = () => setCover().catch(handleError);
  ...
  $("albumMenuBtn").onclick = () => toast("Album options coming next");
}
