...
async function renderAlbums(){
  ...
  for(const album of albums()){
    ...
    strong.textContent=isPrivate?"Private Vault":album;

    const span=document.createElement("span");
    span.textContent=isPrivate?"Private album":plural(photosIn(album).length,"photo");
...
async function importPhotos(files){const list=[...files].filter(f=>f.type.startsWith("image/"));if(!list.length)return toast("Choose one or more pictures");const target=state.album&&!['Library','Favourites'].includes(state.album)?state.album:"Library";for(const file of list)await putPhoto({id:crypto.randomUUID(),name:file.name||"Photo",type:file.type,size:file.size,createdAt:Date.now(),album:target,favourite:false,blob:file});$("picker").value="";toast(`${plural(list.length,"photo")} added`);await refresh()}
...
function renderViewer(){const p=currentPhoto();if(!p)return;if(el.viewerImg.dataset.url)URL.revokeObjectURL(el.viewerImg.src);el.viewerImg.src=urlFor(p);el.viewerImg.dataset.url="1";el.viewerImg.alt=p.name;el.viewerName.textContent=p.name;el.viewerPos.textContent=`${state.viewerIndex+1} of ${state.viewerIds.length}`;el.favBtn.textContent=p.favourite?"Remove from favourites":"Add to favourites";$("coverBtn").classList.toggle("hidden",state.album==="Private");applyTransform()}
...
async function toggleFavourite(){const p=currentPhoto();p.favourite=!p.favourite;await putPhoto(p);await refresh();renderViewer();toast(p.favourite?"Added to favourites":"Removed from favourites")}
async function renamePhoto(){const p=currentPhoto(),name=prompt("Rename photo",p.name)?.trim();if(!name)return;p.name=name;await putPhoto(p);await refresh();renderViewer();toast("Photo renamed")}
...
$("picker").onchange=e=>importPhotos(e.target.files).catch(handleError);$("newAlbumBtn").onclick=()=>{el.albumName.value="";el.albumDialog.showModal();requestAnimationFrame(()=>el.albumName.focus())};el.albumForm.onsubmit=async e=>{e.preventDefault();if(await createAlbum(el.albumName.value))el.albumDialog.close()};$("albumBackBtn").onclick=closeAlbum;$("backBtn").onclick=()=>state.album?closeAlbum():history.back();$("infoBtn").onclick=()=>alert("Ghost Photo Vault v0.3\n\nPhotos are stored in this browser using IndexedDB. This test build is not encrypted yet.");$("selectBtn").onclick=()=>state.selection?exitSelection():enterSelection();$("cancelSelectBtn").onclick=()=>exitSelection();$("moveBtn").onclick=()=>state.selected.size?openMove():toast("Select at least one photo");$("deleteBtn").onclick=()=>deleteMany([...state.selected]).catch(handleError);el.search.oninput=e=>{state.search=e.target.value;renderPhotos()};el.sort.onchange=e=>{state.sort=e.target.value;renderPhotos()};$("closeMoveBtn").onclick=()=>{state.moveSingle=null;el.moveDialog.close()};$("closeViewerBtn").onclick=()=>el.viewer.close();$("viewerMenuBtn").onclick=()=>el.viewerMenu.classList.toggle("hidden");el.viewer.onclick=e=>{if(!el.viewerMenu.contains(e.target)&&e.target.id!=="viewerMenuBtn")el.viewerMenu.classList.add("hidden")};$("prevBtn").onclick=()=>moveViewer(-1);$("nextBtn").onclick=()=>moveViewer(1);$("zoomOutBtn").onclick=()=>setZoom(state.zoom-.5);$("zoomInBtn").onclick=()=>setZoom(state.zoom+.5);$("resetZoomBtn").onclick=()=>{resetTransform();applyTransform()};el.viewerStage.addEventListener("touchstart",touchStart,{passive:true});el.viewerStage.addEventListener("touchmove",touchMove,{passive:false});el.viewerStage.addEventListener("touchend",touchEnd,{passive:true});$("favBtn").onclick=()=>toggleFavourite().catch(handleError);$("renameBtn").onclick=()=>renamePhoto().catch(handleError);$("moveOneBtn").onclick=()=>{const p=currentPhoto();el.viewerMenu.classList.add("hidden");if(p)openMove(p.id)};$("coverBtn").onclick=()=>setCover().catch(handleError);$("deleteOneBtn").onclick=()=>deleteCurrent().catch(handleError);el.viewer.onclose=()=>{if(el.viewerImg.dataset.url){URL.revokeObjectURL(el.viewerImg.src);delete el.viewerImg.dataset.url}el.viewerMenu.classList.add("hidden");resetTransform()}}
